...
exports.getMealPlans = wrapper().getMealPlans;
exports.seed = function(knex) {
    if(!config.env.isProd) {
        return knex('meal_plans').del()
            .then(() => {
                return knex('meal_plans').insert(wrapper().getMealPlans());
            })
            .then(() => {
                return knex('recipe_meal_plans').insert(wrapper().getRecipeMealPlans());
            });
    }
};
